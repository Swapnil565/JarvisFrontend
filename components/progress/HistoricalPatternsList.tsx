...
import { Activity, Brain, Sparkles, MessageCircle, ArrowRight, Check } from 'lucide-react';
...
const dimensionConfig = {
  physical: { icon: Activity, color: '#34D399' },
  mental: { icon: Brain, color: '#00D9FF' },
  spiritual: { icon: Sparkles, color: '#FFB020' },
};
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {pattern.wasActedOn ? (
                    <span className="text-xs px-2 py-0.5 rounded bg-jarvis-muted-green/20 text-jarvis-muted-green flex items-center gap-1">
                ...
                {pattern.outcome && (
                  <p className="text-xs text-jarvis-soft-gray mt-2 flex items-center gap-1">
                    <MessageCircle size={12} /> {pattern.outcome}
                  </p>
                )}
              </div>
              
              {/* View arrow */}
              <ArrowRight size={16} className="text-jarvis-soft-gray" />
            </div>
          </motion.div>
        );
      })}
